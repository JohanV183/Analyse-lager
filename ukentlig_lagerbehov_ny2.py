...
df["planuke"] = (
    pd.to_numeric(
        df["Blande_uke"].astype(str).str.extract(r"(\d+)")[0],
        errors="coerce"
    )
)
...
prod_unik["Frist_blanding"] = pd.to_datetime(
    prod_unik["Frist_blanding"], errors="coerce"
)
prod_unik["Pakke_dato"] = pd.to_datetime(
    prod_unik["Pakke_dato"], errors="coerce"
)
...
# Datasett for plotting
lager_plot = lager_daglig.copy()
lager_plot["Kapasitet"] = KAPASITET_PALLER

# Velg kun perioden som er relevant
lager_plot = lager_plot.loc[start_dato:slutt_dato]

st.line_chart(
    lager_plot[["lagerbeholdning", "Kapasitet"]],
